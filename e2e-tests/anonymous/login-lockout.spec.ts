...
import { MAX_FAILED_ATTEMPTS, LOCKOUT_DURATION_MS } from '../../src/app/login/constants';
...
test.describe('Account lockout functionality', () => {
  ...
  test('should lock account after multiple failed attempts', async ({ page }) => {
    ...
    // Go to login page
    await page.goto(`${process.env.E2E_URL}/login`);
    await expect(page).toHaveURL(`${process.env.E2E_URL}/login`, { timeout: 10000 });

    // Now attempt login with incorrect password multiple times
    // Make exactly MAX_FAILED_ATTEMPTS attempts to trigger lockout
    for (let i = 1; i <= MAX_FAILED_ATTEMPTS; i++) {
      ...
      const errorText = await page.locator('[data-testid="server-error"]').textContent();
    ...
    console.log(`Making one more attempt after ${MAX_FAILED_ATTEMPTS} failed attempts`);
    await page.fill('input[name="email"]', TEST_EMAIL);
    await page.fill('input[name="password"]', 'wrong-password');
    await page.click('button[type="submit"]');
    await page.waitForTimeout(500);
    ...
    await expect(page.locator('[data-testid="server-error"]')).toContainText('Account is locked', { timeout: 10000 });
  ...
  test('should allow login with different user while one account is locked', async ({ page }) => {
    ...
    await expect(page.locator('button:text("Logout")')).toBeVisible({ timeout: 10000 });
  });

  test('should reset failed attempts after successful login', async ({ page, browser }) => {
    ...
    for (let i = 1; i <= attemptsToMake; i++) {
      ...
      await expect(page.locator('[data-testid="server-error"]')).toContainText('Invalid credentials', { timeout: 10000 });
    }
    ...
    // Now we should be able to make MAX_FAILED_ATTEMPTS failed attempts again
    // because the counter should have been reset
    for (let i = 1; i <= attemptsToMake; i++) {
